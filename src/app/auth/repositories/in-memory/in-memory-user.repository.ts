...
import registered from './userregistered.repository' // assert { type: 'json' };
import {userTemp , builtUser } from './userregistered.repository'
...
let usersRegistered = registered[0] || [] ; // readUserRegisteredJSON();
...
export class InMemoryUserRepository implements UserRepository {
  ...
  getUserByEmail(email: string): Promise<User> {
    ...
    return new Promise((resolve, reject) => {
      user ?  resolve(user)  : ( user2 ?  resolve(User.build(user2)) : reject("Invalid Email 2 try ")) ;
    });
  }
     ...
     if(typeof file !== "undefined" || file !== null) {
      ...
      /* for Netlify commented reading userregisteredjson.json
        fs.readFile(fileName, function (err:any, data:any) {
            ...
            */
	      usersRegistered = registered[0]/// readUserRegisteredJSON();
            //  this.resyncUSERSandRegisteredUser();
            }
            else {
              // check user present in In-memonry Array
              const userInM = USERS.find(user => user.email === userIn.email) || userTemp;
